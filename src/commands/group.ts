...
export async function groupCreateAction(
  ...
  try {
    ...
    print.success(
      `Created group "${name}"${skillNames?.length ? ` with ${skillNames.length} skill(s).` : "."}`
    );
  } catch (err: unknown) {
...
export async function groupEnableAction(name: string): Promise<void> {
...
export async function groupDisableAction(name: string): Promise<void> {
  const groups = getAllGroups();
  if (!groups[name]) {
    print.error(`Group "${name}" does not exist.`);
    process.exit(1);
...
export async function groupListAction(): Promise<void> {
  const groups = getAllGroups();
  ...
  for (const gName of groupNames) {
    ...
    console.log(
      `  ${print.bold(gName)} (${skills.length} skill${skills.length !== 1 ? "s" : ""}, ${activeCount} active)`
    );
...
export async function groupOnlyAction(name: string): Promise<void> {
  const groups = getAllGroups();
  if (!groups[name]) {
    print.error(`Group "${name}" does not exist.`);
    process.exit(1);
  ...
  writeRegistry(registry);
  print.success(
    `Activated only group "${name}": ${enabled} skill(s) enabled, all others disabled.`
  );
  await syncAction();
}
