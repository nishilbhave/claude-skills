  ...
  if (parsed) {
    ...
    if (parsed.meta.conflicts?.length) {
      for (const conflict of parsed.meta.conflicts) {
        ...
        if (conflicting) {
          print.error(
            `Cannot enable "${name}": conflicts with active skill "${conflict}".`
          );
          process.exit(1);
        }
      }
    ...
    for (const active of activeSkills) {
      if (active.parsed!.meta.conflicts?.includes(name)) {
        print.error(
          `Cannot enable "${name}": active skill "${active.entry.name}" conflicts with it.`
        );
        process.exit(1);
      }
    }
    ...
    if (parsed.meta.requires?.length) {
      for (const req of parsed.meta.requires) {
        ...
        if (!found) {
          print.warn(
            `Skill "${name}" requires "${req}" which is not active.`
          );
        }
      }
    }
