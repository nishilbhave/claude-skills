...
import { getRegistryPath, getProjectRegistryPath, getClaudeMdPath, getCommandsDir, } from "../utils/paths.js";
import * as print from "../utils/print.js";
function checkRegistryValid(label, regPath) {
...
function checkOrphanedCommands() {
    ...
    const activeNames = new Set(getActiveSkills(registry).map((s) => s.name));
    ...
    for (const file of files) {
        ...
        if (content.startsWith("<!-- claude-skills:managed -->")) {
            ...
            const hasActive = registry.skills.some((s) => s.active && (s.name === cmdName || parseSkillFile(s.path)?.meta.command === cmdName));
            if (!hasActive) {
...
function checkMissingSkillFiles() {
...
export async function doctorAction() {
    ...
    console.log("");
    print.info(`${results.length} checks: ${passes} passed, ${warns} warning(s), ${fails} failed`);
}
