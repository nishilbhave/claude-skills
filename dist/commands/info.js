...
export async function infoAction(name) {
    ...
    if (meta.command)
        console.log(`  ${print.bold("command:")}    /${meta.command}`);
    if (meta.author)
        console.log(`  ${print.bold("author:")}     ${meta.author}`);
    if (meta.tags?.length)
        console.log(`  ${print.bold("tags:")}       ${meta.tags.join(", ")}`);
    if (meta.description) {
